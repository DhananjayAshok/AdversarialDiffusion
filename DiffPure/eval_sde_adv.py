...
def convert_namespace_to_dict(namespace_obj):
    # Convert each nested namespace object to a dictionary
    for name, value in vars(namespace_obj).items():
        if isinstance(value, argparse.Namespace):
            vars(namespace_obj)[name] = convert_namespace_to_dict(value)
...
if __name__ == '__main__':
    ...
    model = SDE_Adv_Model(args, config)
    config_obj = convert_namespace_to_dict(config)
    config_obj['device'] = 'cpu'
